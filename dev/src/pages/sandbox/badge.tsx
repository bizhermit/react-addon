...
const Page: NextPage = () => {
  return (
    <FlexBox $fto="fy" $scroll $padding>
      ...
      <FlexBox $fto="x" $row style={{ gap: 20 }}>
        <Badge $content={<Icon $image="c-check" $color="primary" />} $size="xs">
          <Button $icon="message" $size="xs" />
        </Badge>
        <Badge $content={<Icon $image="c-check" $color="primary" />} $size="s">
          <Button $icon="message" $size="s" />
        </Badge>
        <Badge $content={<Icon $image="c-check" $color="primary" />} $size="m">
          <Button $icon="message" $size="m" />
        </Badge>
        <Badge $content={<Icon $image="c-check" $color="primary" />} $size="l">
          <Button $icon="message" $size="l" />
        </Badge>
        <Badge $content={<Icon $image="c-check" $color="primary" />} $size="xl">
          <Button $icon="message" $size="xl" />
        </Badge>
      </FlexBox>
      <Label>Color</Label>
