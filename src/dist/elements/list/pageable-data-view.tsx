...
import DataView, { DataViewAttributes, dataViewCn, DataViewColumnProps, DataViewHook, useDataView } from "./data-view";
...
export type PageableDataViewHook<T = Data> = DataViewHook<T> & AddinHook;
type Hook = _HookSetter<AddinHook>;

export type PageableDataViewAttributes<T = Data> = Omit<DataViewAttributes<T>, "$hook"> & {
...
interface PageableDataViewFC extends FunctionComponent {
  <T extends {[key: string]: any} = Data>(attrs: PageableDataViewAttributes<T>, ref?: ForwardedRef<HTMLDivElement>): ReactElement<PageableDataViewAttributes<T>>;
}

const PageableDataView: PageableDataViewFC = React.forwardRef<HTMLDivElement, PageableDataViewAttributes<any>>(<T extends {[key: string]: any} = Data>(attrs: PageableDataViewAttributes<T>, ref: ForwardedRef<HTMLDivElement>) => {
  ...
  const sorted = (columnName: string, order: "asc" | "desc" | "", col: DataViewColumnProps) => {
...
export const usePageableDataView = <T extends {[key: string]: any} = any>(): PageableDataViewHook<T> => {
  const dispatch = useRef<Partial<AddinHook>>({});
  const dvHook = useDataView<T>();
  return {
    ...dvHook,
...
.${cn} > .${dataViewCn}-wrap[data-padding="true"] {
  padding-top: 0px;
  padding-bottom: 0px;
}
`}</JsxStyle>;
