...
import { DataViewColumnProps, DataViewClass } from "../data-view";
...
const DataViewReorderColumn: (props?: Props) => DataViewColumnProps = (props) => {
  const range = props?.range ?? "cell", disabled = props?.disabled === true;
  let dataView: DataViewClass;
    ...
    cellInitialize: (cell, params) => {
      if (disabled !== true) {
        const listenerImpl = (isTouch?: boolean) => {
          ...
          if (conElem == null) {
            conElem = dataView.getBodyElement();
            event.addEvent(conElem, "scroll", bodyScrollEvent);
          }
          const bodyScrollTop = dataView.getBodyScrollTop();
          conElem.appendChild(pinchingRowElem);
          pinchingRowIndex = cell.row.index;
          mouseup = (top: number) => {
            dataView.dropMoveRow(pinchingRowIndex, top - bodyScrollTop + dataView.getBodyScrollTop());
          };
          mouseSpace = (top: number) => {
            const t = (lastTop = top) - bodyScrollTop + dataView.getBodyScrollTop();
            dataView.dragMovingRow(pinchingRowIndex, t);
